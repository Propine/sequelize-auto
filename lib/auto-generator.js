...
class AutoGenerator {
    ...
    isNumber(fieldType) {
        return /^(smallint|mediumint|tinyint|int|bigint|float|money|smallmoney|double|numeric|real|oid)/.test(fieldType);
    }
    isBoolean(fieldType) {
    ...
    isString(fieldType) {
        return /^(char|nchar|string|varying|varchar|nvarchar|text|longtext|mediumtext|tinytext|ntext|uuid|uniqueidentifier|date|time|inet|cidr|macaddr|decimal)/.test(fieldType);
    }
    isArray(fieldType) {
